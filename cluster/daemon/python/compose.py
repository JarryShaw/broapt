...
def watch_container():
    try:
        container_id_list = subprocess.check_output(['docker-compose', '--file', DOCKER_COMPOSE,
                                                     'ps', '--quiet'], encoding='utf-8').strip().split()
    except subprocess.CalledProcessError as error:
        traceback.print_exc()
        warnings.warn(f'failed to execute supervisor with exit code {error.returncode}', ComposeWarning)
        return
